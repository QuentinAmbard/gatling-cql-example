...
object SensorAggregation extends App {
  ...
  //You might need to filter on date because we only have a daily resolution here, so if you want to do things at minute or hour precision you'll have to filter data in-memory
  ...
  //Or without map only combineByKey
  val averagePerHive2: RDD[(DailySensor, (Int, Float))] = data.combineByKey(
    (sensorData: SensorAggregation.SensorData) => (1, sensorData.value),
    (acc: (Int, Float), sensorData: SensorAggregation.SensorData) => (acc._1 + 1, sensorData.value + acc._2),
    (acc1: (Int,Float), acc2: (Int, Float)) => (acc1._1+ acc2._1, acc1._2+ acc2._2), 1
  )
