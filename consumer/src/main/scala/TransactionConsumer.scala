...
object TransactionConsumer extends App {
  ...
  val randomUuids = (1 to 30).map(_ => UUID.randomUUID())

  kafkaStream.window(Seconds(3)).map{case (key, value) => {
    value.split(",")
  }}

  kafkaStream.window(Seconds(3), Seconds(3))foreachRDD { (message: RDD[(String, String)], batchTime: Time) => {
      println(s"START NEW RD ${message.count()}")
      message.map { case (k, v) => {
        ...
        //val uuids = UUID.fromString(sensorData.serial_number)
        val r = scala.util.Random
        val uuids = randomUuids(r.nextInt(randomUuids.length))
        //println(uuids)
        sensorDataC(uuids, bucket_ts_date, sensorData.sensor_snapshot, sensorData.hive_num, sensorData.floor, sensorData.wing, sensorData.sensor_type, value, latLong)
      }
      }.saveToCassandra("umbrella", "sensor_data", SomeColumns("sensor_id", "bucket_ts", "time", "hive", "level", "wing", "sensor_type", "value", "latlong"))
    }
    }
  ssc.start()
